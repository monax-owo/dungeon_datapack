...
$execute unless loaded $(pos) run tellraw @a[tag=debug] ["$(pos) is not loaded"]
...
data modify storage dun: temp set from storage util: args[-1]
$execute if data storage dun: temp{tier:1} run data modify block $(pos) LootTable set value "dun:normal_chest"
$execute if data storage dun: temp{tier:2} run data modify block $(pos) LootTable set value "dun:rare_chest"
$execute if data storage dun: temp{tier:2} run data modify block $(pos) lock set value {predicates:{custom_data:{dun:{key:1b}}}}
