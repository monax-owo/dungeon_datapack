...
execute as @n[tag=uuid_sel.aec] on origin at @s run particle dust{color:0,scale:1}
execute as @n[tag=uuid_sel.aec] on origin as @n[distance=0,type=!player] run tag @s add uuid_sel.holder
execute as @n[tag=uuid_sel.aec] on origin run tag @s add uuid_sel.owner

execute as @n[tag=uuid_sel.aec] on origin at @s run tp @n[tag=uuid_sel.holder] ^ ^ ^1
